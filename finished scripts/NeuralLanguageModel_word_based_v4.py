...
print 'Number of overall sequences:' + str(len(tokens))
...
model = Sequential()
model.add(Embedding(input_dim=len(vocab), output_dim=128, input_length=longestSentence, mask_zero=True))
...
for iteration in range(1, 10):
    ...
    for index in range(0, len(tokens)-batchSize, batchSize):
        ...
        print ""
        print "Learning samples between " + str(index) + " and " + str(index+batchSize) + " out of " + str(len(tokens))
        print ""
        ...
        iterationResult = model.fit(trainingInput, trainingOutput, batch_size=100, epochs=1)
